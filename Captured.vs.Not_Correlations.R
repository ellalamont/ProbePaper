...
###### SAME GRAPHS WITH NON CODING GENES REMOVED ######

###########################################################
##################### TPMf PROCESS DATA ###################

my_tpm <- ProbeTest5_tpm_f %>% mutate("Gene" = rownames(ProbeTest5_tpm_f))
names(my_tpm) <- gsub(x = names(my_tpm), pattern = "_S.*", replacement = "")
...
# ggplotly(ScatterCorr)
ggsave(ScatterCorr,
       file = "THP1SpikedCaptured.vs.Broth_TPMf.pdf",
       path = "Figures/Captured.vs.Not_Correlations",
       width = 7, height = 5, units = "in")
...
# ggplotly(ScatterCorr)
ggsave(ScatterCorr,
       file = "THP1SpikedCaptured.vs.NotCaptured_TPMf.pdf",
       path = "Figures/Captured.vs.Not_Correlations",
       width = 7, height = 5, units = "in")
