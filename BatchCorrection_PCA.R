...
my_tpm_bc_t_subset <- my_tpm_bc_t %>% filter(!rownames(my_tpm_bc_t) %in% c("H37Ra_Broth_4_S7", "H37Ra_Broth_5_S8", "H37Ra_Broth_6_S9", "Rv_pH_8.3_R1", "Rv_pH_8.3_R2", "Rv_pH_5.7_R1", "Rv_pH_5.7_R2"))

# Make the actual PCA
my_PCA_tpm_bc <- prcomp(my_tpm_bc_t_subset, scale = TRUE)

# See the % Variance explained
summary(my_PCA_tpm_bc)
summary_PCA_tpm_bc <- format(round(as.data.frame(summary(my_PCA_tpm_bc)[["importance"]]['Proportion of Variance',]) * 100, digits = 1), nsmall = 1) # format and round used to control the digits after the decimal place
summary_PCA_tpm_bc[1,1] # PC1 explains 35.6% of variance
summary_PCA_tpm_bc[2,1] # PC2 explains 13.6% of variance
summary_PCA_tpm_bc[3,1] # PC3 explains 10.5% of variance
...
PCA_BatchCorrected
# ggsave(PCA_BatchCorrected,
#        file = paste0("BatchCorrected_TPM_wRv_v2.pdf"),
#        path = "Figures_preNonCodingRemoval/PCA/wRv",
#        width = 8, height = 5, units = "in")

# Get the top Gene contributors for the PCA (positive or negative values)
top_genes_PC2_bc <- as.data.frame(my_PCA_tpm_bc$rotation) %>%
